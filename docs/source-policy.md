...
| --- | --- | --- | --- | --- | --- |
| `public/database/raw/legacy/Dictionary.txt` | `legacy_dictionary_txt` | `internal` | `low` | `pending` 翻譯候選 | 計畫「既有資料的初始狀態」表列為 `pending` 翻譯候選，理由是「可能與正式詞典翻譯衝突」；其定位為「人工中英對照…匯入待比對，**避免覆蓋正式詞典**」。計畫的 Raw 目錄配置將它放在 `raw/legacy/`（與 `database.csv`、`database.md` 同列）而非 `raw/dictionary/` |
...
T1.2b 已將檔案移入 `public/database/raw/legacy/Dictionary.txt`；正式 `source_key`
指派已併入上方主表，本節僅保留歷史脈絡與約束說明。
